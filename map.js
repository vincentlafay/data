...
var visibles = ['green','yellow','red','blue'];
...
function initMap() {
  ...
  var LatLng = {lat: 48.858053, lng: 2.294289};
  ...
  var icons = {
    zebu: {
      green: {
        icon: urlBaseIcon + 'vert1.png'
      },
      yellow: {
        icon: urlBaseIcon + 'orange1.png'
      },
      red: {
        icon: urlBaseIcon + 'rouge1.png'
      },
	 blue: {
        icon: urlBaseIcon + 'bleu1.png'
      }
    },
    semem: {
      green: {
        icon: urlBaseIcon + 'rennesvert1.png'
      },
      yellow: {
        icon: urlBaseIcon + 'rennesorange1.png'
      },
      red: {
        icon: urlBaseIcon + 'rennesrouge1.png'
      },
	 blue: {
        icon: urlBaseIcon + 'rennesbleu1.png'
      }
    }
  };
  
  function loadMarkers() {
    $.getJSON('https://raw.githubusercontent.com/vincentlafay/data/master/data.json', function(points) {
      $.each(points, function(index, point) {
        ...
        if(point.address != "") {
          content += '   <hr />';
          content += '   <div class="iw-subTitle">Pays / Ville :</div>';
          content += '   <p>' + point.address + '</p>';
        }
        if(point.phone1 != "" || point.phone2 != "" || point.phone3 != "") {
          content += '   <hr />';
          content += '   <div class="iw-subTitle">T&eacute;l&eacute;phone / Mail :</div>';
          if(point.phone1 != "") {
        ...
        if(point.contact != "") {
          content += '   <hr />';
          content += '   <div class="iw-subTitle">Contact :</div>';
          content += '   <p>' + point.contact + '</p>';
        }
        if(point.last_purchase_date != "") {
          content += '   <hr />';
          content += '   <div class="iw-subTitle">Date :</div>';
